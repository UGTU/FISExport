...
namespace Fdalilib
{
    ...
    public sealed class FisProxy
    {
        ...
        /// </summary>
        /// <param name="login">Имя учётной записи в ФИС ЕГЭ</param>
        /// <param name="pass">Пароль</param>
        /// <param name="service">Сервис федеральной системы абитуриентов</param>
        public FisProxy(string login, string pass, IFisProxyService service)
        {
        ...
        /// <summary>
        /// Получает имя учётной записи в ФИС ЕГЭ
        /// </summary>
        public string Login { get; private set; }

        /// <summary>
        /// Получает пароль для досутпа к ФИС ЕГЭ
        /// </summary>
        ...
        private Answer<T, TErr> Request<T, TErr>(Func<XElement, XElement> func, Root param) where T: class where TErr:class
        {
            Contract.Requires(func != null);
            Contract.Requires(param!=null);
            var xParam = Serialize(param);
            LogWriter.WriteLine(xParam);
            LogWriter.Flush();
        ...
        {
            return new Root {AuthData = new AuthData {Login = Login, Pass = Password}};
        }
